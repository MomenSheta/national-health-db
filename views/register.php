...
<body class="bg-slate-100 min-h-screen flex flex-col font-sans">
    ...
    <div class="bg-white border-b border-gray-100 px-8 py-4 flex justify-between items-center shadow-sm h-16">
        ...
        <a href="<?= fixed_path('/login') ?>" class="border border-blue-400 text-blue-500 px-5 py-1.5 rounded-full text-sm font-medium hover:bg-blue-50 transition">Login</a>
    </div>
        ...
        <div class="bg-white p-10 rounded-2xl shadow-xl border border-gray-100 w-full max-w-lg">
            ...
            <form action="<?= fixed_path('/register') ?>" method="POST" class="space-y-5">
                ...
                <div class="text-center text-xs text-slate-500 pt-1">
                    Already have an account? <a href="<?= fixed_path('/login') ?>" class="text-blue-500 hover:underline font-medium">Sign in</a>
                </div>
            </form>
        </div>
    </main>
